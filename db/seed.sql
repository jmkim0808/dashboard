...
--
-- 배출계수(factor)와 환율(fx_rate)은 비워둔 상태로 배포한다.
-- EP8 2-2절 B4(3개 지역 최신 전력 배출계수 공식 고시값)가 확정되기 전에
-- 추측값을 넣으면 그 값으로 산정 로직을 검증하게 되고, 전부 다시 해야 한다.
-- 파일 하단의 등록 템플릿에 실제 고시값을 채워 넣는다.
-- =============================================================================
...
INSERT INTO metric (code, category, name_ko, name_zh, name_vi, unit_standard,
 ...
 ('E-C5','E','에너지 집약도','能源强度','Cường độ năng lượng','TJ/백만','annual','avg',
  'E-C1 ÷ P03. 분자·분모를 각각 합산한 뒤 재계산한다.',
  'public','none','302-3','S2',1,'ratio','E-C1','P03',1.0,155,'2025-01'),
 ('E-C6','E','배출 집약도','排放强度','Cường độ phát thải','tCO2eq/백만','annual','avg',
  '(E-C3 + E-C4) ÷ P03. 분자·분모를 각각 합산한 뒤 재계산한다.',
  'public','none','305-4','S2',1,'ratio','E-C3,E-C4','P03',1.0,156,'2025-01'),
...
-- 배출계수 · 환율 등록 템플릿
--
-- 아래는 의도적으로 주석 처리되어 있다. EP8 2-2절 B3(GWP 버전)·B4(전력 배출계수)가
-- 확정되기 전에 추측값을 넣으면, 그 값으로 산정 로직을 검증하게 되고 전부 다시 해야 한다.
-- G3 게이트(10/10 산출값이 손계산과 일치)를 통과하려면 실제 고시값이어야 한다.
--
-- 확인처
...
--
-- INSERT INTO factor (factor_type, purpose, region, year, value, unit, gwp_set,
--                     source, published_at, version) VALUES
--  ('electricity','emission','KR',   2025, 0.0000000, 'tCO2eq/kWh','AR5','[출처]','[고시일]','v2026.1'),
--  ('electricity','emission','CN-NE',2025, 0.0000000, 'tCO2eq/kWh','AR5','[출처]','[고시일]','v2026.1'),
--  ('electricity','emission','VN',   2025, 0.0000000, 'tCO2eq/kWh','AR5','[출처]','[고시일]','v2026.1'),
--  ('natural_gas','emission','KR',   2025, 0.0000000, 'tCO2eq/Nm3','AR5','[출처]','[고시일]','v2026.1'),
--  ('electricity','heating_value','GLOBAL',2025, 0.0000036,'TJ/kWh',NULL,'[출처]','[고시일]','v2026.1');
--
